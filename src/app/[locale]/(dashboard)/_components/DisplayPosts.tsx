  ...
  useEffect(() => {
    if (branchId || orgName) {
      const fetchPosts = async () => {
        try {
          const url = branchId
            ? `/api/get-verified-posts?branchId=${encodeURIComponent(branchId)}`
            : `/api/get-verified-posts?orgName=${encodeURIComponent(orgName || "")}&placeName=${encodeURIComponent(placeName || "")}`;
    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      {!branchId && !orgName ? (
        <p className="text-center text-gray-600 py-8">
          Select a branch from the sidebar to view posts.
        </p>
      ) : !hasPosts ? (
        <p className="text-center text-gray-600 py-8">
          No posts found for this branch.
        </p>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {posts.map((post) => (
            <div
              ...
              }  ${post.approval ? "bg-white" : "bg-red-300"}`}
            >
              <div className="relative h-40">
                {Array.isArray(post.uploadedPostPhotos) &&
                post.uploadedPostPhotos.length > 0 &&
                post.uploadedPostPhotos[0]?.postUrl ? (
                  <Image
                ...
                <button
                  title="Go to details"
                  onClick={() =>
                    router.push(`/${roleSegment}/posts/${post.id}`)
                  }
